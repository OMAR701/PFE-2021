...
mystruct t;
int[] nbrchar;
t.x=2;t.y=2;t.s="gfdfdfkbjk jdfx kjfdkjg lsjg ljdgl jsl jl";t.z=2;
int nbrch=35;
...
void drawlinkedlist(int nbrLigne,string names[],string values[],int nbrchar,pair shifting,int sizing,int color=0){
	...
	if(nbrchar>6){
		...
		size(3.9cm+(nbrchar-6)*0.37cm+sizing*30);
	}else{
		nbrchar=6;	
		size(3.9cm+sizing*30);
	}
	...
	for(int i=0;i<nbrLigne;++i){
		label(names[i]+" : "+values[i],shift(0,i+1)*h);
	}
}
...
void drawList(string values[][],int nbrchar[],int total,int index,int color=0,mystruct p=null,int pp=0){
	...
	if(total!=0){
		if(color==3 || index==0 && p==null)
			clr=1;
		drawlinkedlist(nbr,names,values[0],nbrchar[0],(0,0),0,clr);
		...
		for(int i=1;i<total;++i){
			...
			if(color==1 && i==total-1 || i==index && p==null){
				...
				clr=2;
				drawlinkedlist(nbr,names,mystructTostring(p),pp,(6*i+(s-6*i)*0.5,2+names.length),4*i,clr);
				clr=0;
			}
			for(int j=0;j<i;++j){
				s=s+nbrchar[j];
			}
		drawlinkedlist(nbr,names,values[i],nbrchar[i],(6*i+(s-6*i)*0.5,0),4*i,clr);
		}
	}
	s=0;
	...
	drawNull(6*total+(s-6*total)*0.5,4*(total-1)+1);
	if(p!=null && color==2 || index==nbrchar.length && p!=null){
		s=s+pp;
		clr=2;
		drawlinkedlist(nbr,names,mystructTostring(p),pp,6*(total)+(s-6*(total))*0.5,4*(total)+1,clr);
	}else if(p!=null && color==4 || index==0 && p!=null){
		clr=2;
		drawlinkedlist(nbr,names,mystructTostring(p),pp,(-6-(pp-6)*0.5,0),4*(total)+1,clr);
	}
}
void pushList(){
	mystruct[] m=fileRead();
